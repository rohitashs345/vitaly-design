class FacetFiltersForm extends HTMLElement {
  constructor() {
    ...
    //SV-filters
    const UpdatePriceFilterValues = (e) => {
      let $prices = this.querySelector("form").querySelectorAll(
        "[data-price-checkbox]:checked"
      );

      let min = $prices.length ? $prices[0].dataset.min : "";
      let max = $prices.length ? $prices[$prices.length - 1].dataset.max : "";

      let $details = e.target.closest("details");
      let $min = $details.querySelector('[name="filter.v.price.gte"]');
      if ($min) {
        $min.value = min;
        $min.dispatchEvent(new Event("change", { bubbles: true }));
      }

      let $max = $details.querySelector('[name="filter.v.price.lte"]');
      if ($max) {
        $max.value = max;
    ...
    const RenderPriceFilterCheckboxes = () => {
      ...
      let min = parseFloat(urlParams.get("filter.v.price.gte"));
      let max = parseFloat(urlParams.get("filter.v.price.lte"));

      let checkboxes = document.querySelectorAll("[data-price-checkbox]");
      checkboxes.forEach(($item) => {
        // if price filter active, set checkboxes
        let item_min = parseFloat($item.dataset.min);
        let item_max = parseFloat($item.dataset.max);
        // This actualy will never check the last option. Since the code base has item_max to 500 and the max item is 230.
        if (item_min >= min && item_max <= max) {
          $item.checked = true;
        }
      });
    };
    ...
    this.querySelector("form").addEventListener("change", (e) => {
      let $target = e.target;
      let isPriceCheckbox = $target.getAttribute("name") == "price-checkbox";
      if (isPriceCheckbox) {
        e.preventDefault();
        e.stopPropagation();
        e.stopImmediatePropagation();
        UpdatePriceFilterValues(e);
      }
    });

    RenderPriceFilterCheckboxes();
    document.addEventListener("filter:updated", (e) => {
      setTimeout(RenderPriceFilterCheckboxes, 0);
    });
  ...
  createSearchParams(form) {
    const formData = new FormData(form);
     ...
     for (var pair of formData.entries()) {
      let key = pair[0];
      let value = pair[1];
      if (key.indexOf("price-checkbox") !== -1) {
        formData.delete(key);
      }
    }
